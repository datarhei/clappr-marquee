...
"use strict";
eval("Object.defineProperty(exports,\"__esModule\",{value:true});var _getIterator2=__webpack_require__(/*! babel-runtime/core-js/get-iterator */ \"./node_modules/babel-runtime/core-js/get-iterator.js\");var _getIterator3=_interopRequireDefault(_getIterator2);var _typeof2=__webpack_require__(/*! babel-runtime/helpers/typeof */ \"./node_modules/babel-runtime/helpers/typeof.js\");var _typeof3=_interopRequireDefault(_typeof2);var _classCallCheck2=__webpack_require__(/*! babel-runtime/helpers/classCallCheck */ \"./node_modules/babel-runtime/helpers/classCallCheck.js\");var _classCallCheck3=_interopRequireDefault(_classCallCheck2);var _createClass2=__webpack_require__(/*! babel-runtime/helpers/createClass */ \"./node_modules/babel-runtime/helpers/createClass.js\");var _createClass3=_interopRequireDefault(_createClass2);var _possibleConstructorReturn2=__webpack_require__(/*! babel-runtime/helpers/possibleConstructorReturn */ \"./node_modules/babel-runtime/helpers/possibleConstructorReturn.js\");var _possibleConstructorReturn3=_interopRequireDefault(_possibleConstructorReturn2);var _inherits2=__webpack_require__(/*! babel-runtime/helpers/inherits */ \"./node_modules/babel-runtime/helpers/inherits.js\");var _inherits3=_interopRequireDefault(_inherits2);var _clappr=__webpack_require__(/*! clappr */ \"clappr\");var _style=__webpack_require__(/*! ./public/style.scss */ \"./src/public/style.scss\");var _style2=_interopRequireDefault(_style);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}var VERSION='0.1.0';var Marquee=function(_UICorePlugin){(0,_inherits3.default)(Marquee,_UICorePlugin);function Marquee(){(0,_classCallCheck3.default)(this,Marquee);return(0,_possibleConstructorReturn3.default)(this,_UICorePlugin.apply(this,arguments));}Marquee.prototype.bindEvents=function bindEvents(){var _this2=this;this.listenTo(this.core,_clappr.Events.CORE_ACTIVE_CONTAINER_CHANGED,function(){_this2.core.activeContainer.$el.append(_this2.el);_this2.playback=_this2.core.getCurrentPlayback();_this2.listenTo(_this2.playback,_clappr.Events.PLAYBACK_PLAY,function(){_this2.isPlaying=true;_this2.state.recalculate=true;_this2.show();});_this2.listenTo(_this2.playback,_clappr.Events.PLAYBACK_STOP,function(){_this2.isPlaying=false;_this2.show();});});this.listenToOnce(this.core,_clappr.Events.CORE_READY,this.onCoreReady);};Marquee.prototype.onCoreReady=function onCoreReady(){this.hasData=false;this.isPlaying=false;this.state={container:null,div:null,position:0,interval:null,recalculate:true,windowWidth:0,textWidth:0};this.text='';this.speed=10;this.fps=25;this.direction='left';this.style={};this.pauseOnHover=true;var cfg=this.core.options.marqueeConfig||{};if('text'in cfg){this.text=cfg.text;}if('speed'in cfg){this.speed=parseInt(cfg.speed);}if('fps'in cfg){this.fps=parseInt(cfg.fps);}if('direction'in cfg){if(cfg.direction==='right'){this.direction='right';}}if('style'in cfg){if((0,_typeof3.default)(cfg.style)==='object'){this.style=cfg.style;}}if('pauseOnHover'in cfg){this.pauseOnHover=!!cfg.pauseOnHover;}this.create();};Marquee.prototype.create=function create(){var _this3=this;var container=document.createElement('div');container.style.position='absolute';container.style.width='100%';var div=document.createElement('div');div.style.position='absolute';div.style.left='0px';var re=/\\[([^\\]]+)\\]\\(([^\\)]+)\\)/g;var matches=[];var match=null;while((match=re.exec(this.text))!==null){// This is necessary to avoid infinite loops with zero-width matches\nif(match.index===re.lastIndex){re.lastIndex++;}var link=document.createElement('a');link.href=match[2];link.setAttribute('target','_blank');link.append(document.createTextNode(match[1]));matches.push({element:link,start:match.index,end:re.lastIndex});}if(matches.length==0){div.append(document.createTextNode(this.text));}else{var pos=0;var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=(0,_getIterator3.default)(matches),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){var m=_step.value;var _text=this.text.substring(pos,m.start);if(_text.length!=0){div.append(document.createTextNode(_text));}div.append(m.element);pos=m.end;}}catch(err){_didIteratorError=true;_iteratorError=err;}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return();}}finally{if(_didIteratorError){throw _iteratorError;}}}var text=this.text.substring(pos);if(text.length!=0){div.append(document.createTextNode(text));}}container.append(div);this.state.div=div;this.state.container=container;this.$el.html(this.state.container);if(this.pauseOnHover==true){this.$el.mouseover(function(e){e.preventDefault();_this3.pause();});this.$el.mouseleave(function(e){e.preventDefault();_this3.start();});}for(var key in this.style){this.$el.css(key,this.style[key]);}};Marquee.prototype.update=function update(){this.state.div.style.left=Math.round(this.state.position)+'px';};Marquee.prototype.show=function show(){if(this.text.length!=0&&this.isPlaying==true){this.$el.show();this.start();}else{this.$el.hide();this.pause();}return;};Marquee.prototype.start=function start(){var _this4=this;if(this.state.recalculate==true){// The width of the player (w)\nthis.state.windowWidth=this.$el.width();// The width of the text (t)\nthis.state.textWidth=this.state.div.clientWidth;// 0        t                            w+t      w+2t\n// |--text--|------------player-----------|--text--|\nthis.state.container.style.width=this.state.windowWidth+2*this.state.textWidth+'px';this.state.container.style.left=-this.state.textWidth+'px';this.state.recalculate=false;if(this.direction=='right'){this.state.position=0;}else{this.state.position=this.state.windowWidth+this.state.textWidth;}}if(this.state.interval!==null){return;}this.state.interval=setInterval(function(){if(_this4.direction=='right'){_this4.state.position+=_this4.speed/_this4.fps;if(_this4.state.position>=_this4.state.windowWidth+_this4.state.textWidth){_this4.state.position=0;}}else{_this4.state.position-=_this4.speed/_this4.fps;if(_this4.state.position<=0){_this4.state.position=_this4.state.windowWidth+_this4.state.textWidth;}}_this4.update();},1000/this.fps);};Marquee.prototype.pause=function pause(){clearInterval(this.state.interval);this.state.interval=null;};(0,_createClass3.default)(Marquee,[{key:'name',get:function get(){return'marquee';}},{key:'attributes',get:function get(){return{'class':this.name,'data-marquee':''};}},{key:'events',get:function get(){return{};}}],[{key:'version',get:function get(){return VERSION;}}]);return Marquee;}(_clappr.UICorePlugin);exports.default=Marquee;module.exports=exports['default'];\n\n//# sourceURL=webpack://Marquee/./src/main.js?");
